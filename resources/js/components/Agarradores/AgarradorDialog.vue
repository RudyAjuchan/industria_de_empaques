<template>
    <v-dialog v-model="open" max-width="400">
        <v-card>
            <v-card-title>
                {{ form.id ? 'Editar tipo de agarrador' : 'Nuevo tipo de agarrador' }}
            </v-card-title>
    ...
    methods: {
        ...
        async save() {
            ...
            try {
                let res
                if (this.form.id) {
                    res = await axios.put(`/agarrador/${this.form.id}`, this.form)
                } else {
                    res = await axios.post('/agarrador', this.form)
                }
