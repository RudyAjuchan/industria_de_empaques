<template>
    <v-dialog v-model="open" max-width="400">
        <v-card>
            <v-card-title>
                {{ form.id ? 'Editar tipo de papel' : 'Nuevo tipo de papel' }}
            </v-card-title>
    ...
    methods: {
        ...
        async save() {
            ...
            try {
                let res
                if (this.form.id) {
                    res = await axios.put(`/tipo-papel/${this.form.id}`, this.form)
                } else {
                    res = await axios.post('/tipo-papel', this.form)
                }
